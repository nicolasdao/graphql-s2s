...
var wrapVariablesInDoubleQuotes = function wrapVariablesInDoubleQuotes() {
    var jsonStr = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    return (jsonStr.match(/": \$[^\s|}|,]*/g) || []).reduce(function (acc, x) {
        var v = x.replace('": ', '');
    ...
    return !arg ? arg : chain((arg || '').replace(/^\(/g, '{').replace(/\)$/g, '}') // Remove wrapping
        ...
        return chain(arg.match(/:(\s*?)"(.*?)"/g)).next(function (strValues) {
            ...
            ? strValues.reduce(function (a, v) {
                return chain({ alias: '--' + newShortId() + '--', value: v.replace(/^:/, '') }).next(function (_ref4) {
        ...
        var arg = _ref5.arg,
            valueAliases = _ref5.valueAliases;
        return chain(arg.match(/:(\s*?)'(.*?)'/g)).next(function (strValues) {
            return strValues // Escapes prop value wrapper in ''
            ? strValues.reduce(function (a, v) {
                return chain({ alias: '--' + newShortId() + '--', value: v.replace(/^:/, '') }).next(function (_ref6) {
    ...
    ).next(function (_ref12) {
        var arg = _ref12.arg,
            props = _ref12.props,
            valueAliases = _ref12.valueAliases;
        ...
        }, { arg: arg, propAliases: [] })).next(function (_ref13) {
            var arg = _ref13.arg,
                propAliases = _ref13.propAliases;
    ...
    return chain(removeMultiSpaces(escapeGraphQlSchema(query).replace(/#(.*?)_cr_/g, '').replace(/(_cr_|,)/g, ' ')).trim()).next(function (q) {
        return q.match(/^[q|Q]uery /) ? q.replace(/^[q|Q]uery(.*?){/, '{') : q.match(/^[m|M]utation /) ? q.replace(/^[m|M]utation(.*?){/, '{') : q;
    }).val
    ...
    return chain(query.match(/{([^{]*?)}/g)).next(function (blocks) {
        return blocks ? chain(blocks.reduce(function (q, block) {
            return chain('__' + newShortId() + '__').next(function (alias) {
                q.blockAliases.push({ alias: alias, block: block.replace(/({|})/g, '').trim().replace(/\s*:\s*/g, ':') });
     ...
     */
    ();
};var getQueryOrMutationAST = function getQueryOrMutationAST(query, schemaAST) {
    ...
    return chain(throwError(!query, 'Error in method \'getQueryAST\': Parameter \'query\' is required.')).next(function () {
        ...
        return chain(schemaAST.find(function (x) {
            ...
            return chain(astParse(query)).next(function (ast) {
...
var getQueryAST = function getQueryAST() {
    var query = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
    var schemaAST = arguments[1];

    var queryType = chain(query.trim()).next(function (q) {
        return q.match(/^{|[q|Q]uery\s/) ? 'Query' : q.match(/^{|[m|M]utation\s/) ? 'Mutation' : null;
    }).val();
    if (!queryType) throw new Error('Invalid GraphQL query exception. Only Query and Mutation are currently supported. This query can\'t be parsed by \'graphqls2s\': ' + query);
...
var getTypeDetails = function getTypeDetails(t, metadata) {
	return chain(t.indexOf('<') > 0).next(function (isGen) {
		return { originName: t, isGen: isGen, name: isGen ? getAliasName(t, metadata) : t };
		...
		return _(blockParts).reduce(function (a, part) {
			...
			if (p.indexOf('#') == 0) a.comments.push(p);else {
				...
				var propDetails = paramsMatch ? chain(prop.split(paramsMatch[0])).next(function (parts) {
					return { name: parts[0].trim(), metadata: mData, params: paramsMatch[1], result: getTypeDetails((parts[1] || '').replace(':', '').trim(), metadata) };
				}).val() : chain(prop.split(':')).next(function (parts) {
					return { name: parts[0].trim(), metadata: mData, params: null, result: getTypeDetails((parts[1] || '').trim(), metadata) };
				}).val();
				a.props.push({
...
var createNewSchemaObjectFromGeneric = function createNewSchemaObjectFromGeneric(_ref4, schemaBreakDown) {
