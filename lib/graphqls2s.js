...
var _graphQlQueryTypes = { 'query': 'Query', 'mutation': 'Mutation', 'subscription': 'Subscription'
    ...
    if (ast) {
        ...
        (0, _assign2.default)(output, {
            ...
            paths: function paths(fn) {
                return getQueryASTPath(output, fn);
            }
        });
        return output;
    } else return null;
...
var filterQueryAST = function filterQueryAST() {
    ...
    if (operation.body && predicate) {
        var filteredBody = operation.body.filter(function (x) {
            ...
            return x.properties && x.properties.length > 0 ? (0, _assign2.default)({}, x, { properties: filterQueryAST({ body: x.properties }, predicate, true) }) : x;
        });
...
var detectQueryAST = function detectQueryAST() {
    var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var predicate = arguments[1];
    return operation.body && predicate && (operation.body.some(function (x) {
        return predicate(x);
    }) || operation.body.some(function (x) {
        return detectQueryAST({ body: x.properties }, predicate);
    }));
};

var getQueryASTPath = function getQueryASTPath() {
    var operation = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var predicate = arguments[1];
    var parent = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';

    var prefix = parent ? parent + '.' : parent;
    if (operation.body && predicate) return operation.body.reduce(function (acc, p) {
        if (predicate(p)) acc.push(prefix + p.name);
        if (p.properties) acc.push.apply(acc, (0, _toConsumableArray3.default)(getQueryASTPath({ body: p.properties }, predicate, prefix + p.name)));
        return acc;
    }, []);else return [];
};
