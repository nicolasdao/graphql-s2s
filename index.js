 ...
 *         }]             									Property breakdown
 */
const getBlockProperties = (blockParts) => _(blockParts).reduce((a, part) => {
		const p = part.trim();
		if (p.indexOf('#') == 0)
			a.comments.push(p);
		else {
			...
			const propDetails = paramsMatch 
				? chain(prop.split(paramsMatch[0]))
					.next(parts => ({ name: parts[0].trim(), params: paramsMatch[1], result: getTypeDetails((parts[1] || '').replace(':', '').trim()) })).val()
				: chain(prop.split(':'))
					.next(parts => ({ name: parts[0], params: null, result: getTypeDetails((parts[1] || '').trim()) })).val();
			a.props.push({ 
...
const getSchemaObject = (definitions, typeName, nameRegEx) => 
	_.toArray(_(definitions).filter(d => d.property.indexOf(typeName) == 0)
	.map(d => {
		const nameMatch = d.property.match(nameRegEx);
		if (!nameMatch) throw new Error(`Error in schema: ${typeName} with missing name.`);
		const name = nameMatch[1].trim().split(' ')[0];
		const typeMatch = name.match(genericTypeRegEx);
		const type = typeMatch ? typeMatch[1] : null;
		const inheritsMatch = d.property.match(inheritsRegex);
		const superClass = inheritsMatch 
			? inheritsMatch[0].replace('inherits ', '').replace(',', '').trim()
			: null;
		const implementsMatch = d.property.match(implementsRegex);
		const interface = implementsMatch 
			? implementsMatch[0].replace('implements ', '').replace('{', '').split(',').map(x => x.trim().split(' ')[0]) 
			: null;

		// if (typeName == 'input' && name.indexOf('PostQueryWhere') >= 0) {
		// 	log("DEF");
		// 	log(d);
		// 	log("NAME");
		// 	log(name);
		...
		return { type: typeName.toUpperCase(), name: name, genericType: type, blockProps: getBlockProperties(d.block), inherits: superClass, implements: interface  }
	}));

const getInterfaces = definitions => getSchemaObject(definitions, 'interface', interfaceNameRegEx);

const getAbstracts = definitions => getSchemaObject(definitions, 'abstract', abstractNameRegEx);

const getTypes = definitions => getSchemaObject(definitions, 'type', typeNameRegEx);

const getInputs = definitions => getSchemaObject(definitions, 'input', inputNameRegEx);

const getEnums = definitions => getSchemaObject(definitions, 'enum', enumNameRegEx);
...
const getInterfaceWithAncestors = (interface, schemaObjects) => {
	if (memoizedInterfaceWithAncestors[interface]) return memoizedInterfaceWithAncestors[interface];
	const interfaceObj = schemaObjects.filter(x => x.name == interface).first();
	if (!interfaceObj) throw new Error(`Schema error: interface ${interface} is not defined.`);
	if (interfaceObj.type != "INTERFACE") throw new Error(`Schema error: Schema property ${interface} is not an interface. It cannot be implemented.`);
	...
	memoizedInterfaceWithAncestors[interface] = interfaceWithAncestors;
...
const parseSchemaObjToString = (comments, type, name, implements, blockProps) => [
		`${comments && comments != '' ? `\n${comments}` : ''}`, 
		`${type.toLowerCase()} ${name}${implements && implements.length > 0 ? ` implements ${implements.join(', ')}` : ''} { `,
...
const getSchemaParts = graphQlSchema => chain(_([getInterfaces, getAbstracts, getTypes, getInputs, getEnums]
		.reduce((objects, getObjects) => objects.concat(getObjects(getSchemaBits(graphQlSchema))), [])))
...
module.exports = {
	getSchemaParts: (graphQlSchema) => chain(resetMemory())
		.next(v => getSchemaParts(removeGraphMetadata(graphQlSchema)))
